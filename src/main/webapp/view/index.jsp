...
<script>
    ...
    if('WebSocket' in window){
        webSocket=new WebSocket("ws://"+url+"/chatSever/"+${user.id});
    }else{
    ...
    */
    ...
    function showChat(message){
        ...
        var isSef = '${user.id}' == message.from.userid ? "am-comment-flip" : "";   //如果是自己则显示在右边,他人信息显示在左边

        var pic;
        var url = message.from.userid + "/head";
        $.getJSON(url, function (data) {
            pic = "/images/head/" + data.uImage;
    ...
    <%--var wsServer = null;--%>
    <%--var ws = null;--%>
    <%--wsServer = "ws://" + window.location.host+ "/chatServer/${user.id}";--%>
